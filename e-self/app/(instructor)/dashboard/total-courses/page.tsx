...
import {
    ...
    TrashIcon,
    PlusIcon,
    // BookOpenIcon, // Removed unused import
    UserCircleIcon,
    StarIcon,
} from '@heroicons/react/24/outline';
import { BookOpenIcon as SolidBookOpenIcon } from '@heroicons/react/24/solid'; // Solid icon is used
...
const getLocalCourses = (): DisplayCourse[] => {
    ...
    try {
        ...
        // FIX: Replace 'any' with Partial<DisplayCourse> and assert the result
        return JSON.parse(savedCourses).map((course: Partial<DisplayCourse>) => ({
            ...
            status: course.status || 'unfinished', // Default status if missing
             // Ensure all DisplayCourse fields are accounted for, even if with defaults
        }) as DisplayCourse); // Assert the result is DisplayCourse
    } catch (e) {
...
const CourseCard: React.FC<DisplayCourse> = ({ // Use DisplayCourse type
    ...
    // FIX: Add 'relative' class to the parent div for the Image component
    <div className="bg-white rounded-xl shadow-sm overflow-hidden transition transform duration-200 hover:scale-[1.01] hover:shadow-md hover:ring-1 hover:ring-red-100 relative w-full h-40"> {/* Added relative, w-full, h-40 for Image fill */}
        ...
        <Image
            ...
            sizes="(max-width: 640px) 100vw, (max-width: 768px) 50vw, (max-width: 1024px) 33vw, 25vw" // Improve performance with sizes
            className="rounded-t-xl" // Apply styling if needed
        />
        {/* Content below the image */}
        <div className="p-4 relative z-10 bg-white h-[calc(100%-10rem)]"> {/* Added relative z-10 and calculated height to push content below image */}
